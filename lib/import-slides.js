...
/**
 * Import slides from Google Slides via the Slides API (authenticated).
 * Requires: GOOGLE_APPLICATION_CREDENTIALS (service account) or GOOGLE_API_KEY.
 * The presentation must be shared with the service account email.
 */
async function importFromGoogleSlides(config) {
  ...
  // ADC (gcloud auth application-default login), service account, or API key
  try {
    ...
    if (process.env.GOOGLE_API_KEY) {
      ...
      throw new Error(
        ...
        '  Or set GOOGLE_APPLICATION_CREDENTIALS or GOOGLE_API_KEY.'
      );
    }
  }
  ...
  console.log(chalk.dim('  Authenticating with Google Slides API…'));
